...
class Skill1:
    ...
    def draw(self):
        self.image.clip_draw(self.frame * skill_data['skill1']['draw']['left'],
                             ...
                             skill_data['skill1']['draw']['width'], skill_data['skill1']['draw']['height'], self.x, self.y,
    ...
    def collide_check_func(self, targetList1, targetList2):
        if self.attack_check == True:
            if self.state == self.ATTACK:
                if self.total_frame >= self.attack_frame:
                    for target in targetList1:
                        if self.collide(target.get_hit_bb()) == True:
                            target.skill_hit(self.damage)
...
class Skill2:
    ...
    def draw(self):
        self.image.clip_draw(self.frame * skill_data['skill2'][self.state]['left'], skill_data['skill2'][self.state]['bottom'],
                             ...
                             self.x, self.y + skill_data['skill2'][self.state]['ypos'],
...
class Heal:
    ...
    def draw(self):
        self.image.clip_draw(self.frame * skill_data['heal']['draw']['left'], skill_data['heal']['draw']['bottom'],
                             skill_data['heal']['draw']['width'], skill_data['heal']['draw']['height'], self.x, self.y,
                             skill_data['heal']['draw']['xsize'], skill_data['heal']['draw']['ysize'])

    def collide_check_func(self, friendList, monsterList):
        pass
